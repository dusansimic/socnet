...
twitch_graph = SimpleGraph(28281)

for line in eachline("/twitch_data/musae_ENGB_edges.csv")
  i, j = split(line, ",")
...
core_num = core_number(twitch_graph)
save_object("/out/twitch_core_num.jld2", core_num)
println("$(now()): Calculating degree...")
deg = degree(twitch_graph)
save_object("/out/twitch_degree.jld2", deg)
println("$(now()): Calculating betweenness...")
betw = betweenness_centrality(twitch_graph)
save_object("/out/twitch_betweenness.jld2", betw)
println("$(now()): Calculating closeness...")
clos = closeness_centrality(twitch_graph)
save_object("/out/twitch_closeness.jld2", clos)

println("$(now()): Calculating Pearson correlations...")
correlations = cor([core_num deg betw clos], dims=1)
save_object("/out/twitch_pearson_correlation.jld2", correlations)
...
# Plots.svg(pearson_heatmap, "/out/twitch_pearson_correlation.svg")
...
# Plots.svg(correlation_scatter, "/out/twitch_correlation_scatter")
...
save_object("/out/twitch_spearman_correlation.jld2", spearman_correlation)
...
# Plots.svg(spearman_heatmap, "/out/twitch_spearman_correlation.svg")
...
characteristics = macroscopic_characteristics_analysis(twitch_graph)
save_object("/out/twitch_characteristics.jld2", characteristics)
...
# Plots.svg(characteristics_plots, "/out/twitch_characteristics.svg")
